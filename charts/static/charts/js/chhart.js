...
async function loadChartData() {
    ...
    // Fetch the new data
    const newApiUrl = "http://127.0.0.1:8880/api/get_xtimes_preds/btc";
    const newData;
    try {
        const response = await fetch(newApiUrl);
        const result = await response.json();
        const newData = result.result.map(d => ({ date: new Date(+d[0] - timezoneOffset ), value: +d[1] }));
    } catch (error) {
...
function updateChartDataPeriodically() {
    ...
    setTimeout(updateChartDataPeriodically, 8000); 
}
