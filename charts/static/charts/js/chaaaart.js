...
/**
        ...
        data.forEach((item, index) => {
          if (!item) {
            console.log(`Item at index ${index} is null.`);
          } else if (!item.time || !item.open || !item.high || !item.low || !item.close) {
            console.log(`Item at index ${index} is missing one or more properties:`, item);
          }
        });
*/
...
/**
        // Update line series with the same data
        lineSeries.setData(uniqueData);

        // Add a price line at the last close price
        const lastBarData = uniqueData[uniqueData.length - 1]; // Use the last data point from the array
        if (lastBarData) {
            lineSeries.createPriceLine({
                price: lastBarData.close,
                color: 'rgba(55, 111, 166, 0.4)',
                lineWidth: 1,
                lineStyle: LightweightCharts.LineStyle.Dotted,
                axisLabelVisible: true,
                title: `Close ${lastBarData.close}`  // change this to the information you want to display
            });
        }
*/
    ...
    function updateBinanceDataPeriodically() {
        loadBinanceData();
        setTimeout(updateBinanceDataPeriodically, 6.5 * 1000);
    }
    
    function updateServerDataPeriodically() {
        loadServerData();
        setTimeout(updateServerDataPeriodically, 30 * 1000);
    }
